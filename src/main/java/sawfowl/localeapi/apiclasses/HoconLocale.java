...
public class HoconLocale extends AbstractLocale {
	...
	public HoconLocale(LocaleService localeService, Logger logger, Path path, String pluginID, String locale) {
		...
		configLoader = HoconConfigurationLoader.builder().emitJsonCompatible(true).emitComments(false).defaultOptions(localeService.getConfigurationOptions()).path(this.path).build();
		reload();
	}
